...
int get_next_token(char *, char **);
int iswhite(char);
int my_strncpy(char *, const char *, int);
int my_strlen(const char *);
int my_strequal(const char *, const char *);
int my_strncat(char *, const char *, int);
...
int execute(command_t* p_cmd)
{
  ...
  char fullpath_pipe[MAXSTRLEN];
  int fds[2];
  int child_process_status;
  command_t pipe_to;  
  int pipe_argc;
  /**
  Loop to tell if the command is a pipe and if so where the location 
  of command  will be piped to. 
  **/
 for(int i = 0; i<p_cmd->argc; i++){  
   if(p_cmd->argv[i][0]== '|'){
     do_pipe = 1;
     pipe_pos = i;
     pipe_argc = p_cmd->argc - pipe_pos -1;
     p_cmd->argv[i]=NULL;
   }
 }
 
  //Pipe has been found. Create temporary p_cmd structure to save arguments
  if(do_pipe==1){  
  //The command being typed to starts at the next position after a '|' has been found 
    pipe_to.argv = (char **) malloc((pipe_argc+1) * sizeof(char *));
    pipe_to.name = malloc(sizeof(my_strlen(p_cmd->argv[pipe_pos+1])+1));
  /**
  This loops goes through p_cmd, mallocs and copies values accordingly.
  **/
  for(int i = 0; p_cmd->argv[pipe_pos+1]!= NULL;i++ ){
    pipe_to.argv[i] = malloc(sizeof(my_strlen(p_cmd->argv[pipe_pos+1])));
    my_strncpy(pipe_to.argv[i],p_cmd->argv[pipe_pos+1],my_strlen(p_cmd->argv[pipe_pos+1])+1);
    pipe_pos++;
  }
my_strncpy(pipe_to.name, pipe_to.argv[0], my_strlen(pipe_to.argv[0]));

find_fullpath(fullpath,p_cmd);
find_fullpath(fullpath_pipe,&pipe_to);
...
dup(fds[0]);
close(fds[1]);
execv(fullpath_pipe,pipe_to.argv);
}
close(fds[0]);
close(fds[1]);
